<script lang="ts">
	...
	const changeDialogState = (dialog: "form"|"delete", state: boolean, point: Points|null = null) => {
		selectedPoint = point
...
<AddPoint {isOpen} onClose={() => changeDialogState("form", false)} point={selectedPoint} {form} formAction={'?/editPoint'} />
...
<AlertDialog.Root open={isDeleteAlertOpen} onOpenChange={() => changeDialogState("delete", false) } >
	<AlertDialog.Content>
		...
		<form action="?/deletePoint" method="POST" use:enhance>
			<input type="text" name="id" id="id" class="hidden" value={selectedPoint?.id}>
